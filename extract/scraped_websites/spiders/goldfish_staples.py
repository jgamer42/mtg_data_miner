...
class GoldFishSpider(scrapy.Spider):
    ...
    staples_urls = [f"https://www.mtggoldfish.com/format-staples/{game_format}" for game_format in formats]
    ...
    def parse(self,response):
        ...
        view_more_links = response.xpath("//a[@class='staples-view-more']/@href").getall()
        for link in view_more_links:
            yield response.follow(url=link,callback=self.view_more_section,cb_kwargs={"format":format})
    ...
    def get_card_info(self,format,source,card):
        ...
        item["image"] = row_loaded.xpath("//td[@class='col-card']/span/a/@data-full-image")
        item["played_number"] = row_loaded.xpath("//td[not(contains(text(),'%')) and @class='text-right' and not(following-sibling::td)]/text()")
        try:
            mana_cost = row_loaded.xpath("//td[@class='responsive-column']/span/@aria-label")
