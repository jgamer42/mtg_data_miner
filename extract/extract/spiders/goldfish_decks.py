...
class GoldFishSpider(scrapy.Spider):
    ...
    staples_urls = [f"https://www.mtggoldfish.com/metagame/{game_format}" for game_format in formats]
    ...
    def parse(self,response):
        ...
        decks = response.xpath("//div[@class='archetype-tile-title']/span[@class='deck-price-paper']/a/@href").getall()
        meta = response.xpath("//div[@class='archetype-tile-statistic-value' and contains(text(),'%')]").extract()

        for i,deck_link in enumerate(decks):
            link = deck_link.replace("#paper","#arena")
            try:
                yield response.follow(url=link,callback=self.deck_section,cb_kwargs={"format":format,"meta_info":meta[i]})
            except:
                yield response.follow(url=link,callback=self.deck_section,cb_kwargs={"format":format})
    ...
    def card_section(self,raw_card):
        ...
        rarity = loaded_card.xpath("//td[@class='price-arena-rarity text-right']/text()")
